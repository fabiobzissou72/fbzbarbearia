  ...
  if (authLoading || loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-fbz-primary" />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
          ...
          <>
            ...
            <div className="flex space-x-2 border-b border-border">
              <button
                ...
                className={`px-6 py-3 font-medium transition-colors relative ${tabAtiva === 'servicos' ? 'text-fbz-primary dark:text-fbz-accent' : 'text-muted-foreground'}`}
              >
                SERVIÇOS ({servicos.length})
                {tabAtiva === 'servicos' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-fbz-primary dark:bg-fbz-accent" />}
              </button>
              <button
                onClick={() => setTabAtiva('produtos')}
                className={`px-6 py-3 font-medium transition-colors relative ${tabAtiva === 'produtos' ? 'text-fbz-primary dark:text-fbz-accent' : 'text-muted-foreground'}`}
              >
                PRODUTOS ({produtos.length})
                {tabAtiva === 'produtos' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-fbz-primary dark:bg-fbz-accent" />}
              </button>
              <button
                onClick={() => setTabAtiva('planos')}
                className={`px-6 py-3 font-medium transition-colors relative ${tabAtiva === 'planos' ? 'text-fbz-primary dark:text-fbz-accent' : 'text-muted-foreground'}`}
              >
                PACOTES ({planos.length})
                {tabAtiva === 'planos' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-fbz-primary dark:bg-fbz-accent" />}
              </button>
            </div>
            ...
            <div className="space-y-3">
              {tabAtiva === 'servicos' && servicos.map((s) => (
                <div key={s.id} onClick={() => toggleCarrinho({ id: s.id, tipo: 'servico', nome: s.nome, preco: s.preco, duracao: s.duracao_minutos })} className={`card cursor-pointer ${itemNoCarrinho(s.id, 'servico') ? 'border-fbz-primary bg-fbz-primary/5' : ''}`}>
                  <div className="flex justify-between items-center">
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 mt-2">
                        <span className="text-lg font-bold text-fbz-gold">R$ {s.preco.toFixed(2)}</span>
                        <span className="flex items-center text-sm text-muted-foreground"><Clock className="w-4 h-4 mr-1" />{s.duracao_minutos} min</span>
                      </div>
                    </div>
                    <button className={`px-6 py-2 rounded-lg font-semibold transition-colors ${itemNoCarrinho(s.id, 'servico') ? 'bg-fbz-primary text-white' : 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-2 border-fbz-primary/20'}`}>
              ...
              {tabAtiva === 'produtos' && produtos.map((p) => (
                <div key={p.id} onClick={() => toggleCarrinho({ id: p.id, tipo: 'produto', nome: p.nome, preco: p.preco })} className={`card cursor-pointer ${itemNoCarrinho(p.id, 'produto') ? 'border-fbz-primary bg-fbz-primary/5' : ''}`}>
                  <div className="flex justify-between items-center">
                    <div className="flex-1">
                      <h3 className="font-bold text-lg">{p.nome}</h3>
                      {p.descricao && <p className="text-sm text-muted-foreground mt-1">{p.descricao}</p>}
                      <span className="text-lg font-bold text-fbz-gold mt-2 inline-block">R$ {p.preco.toFixed(2)}</span>
                    </div>
                    <button className={`px-6 py-2 rounded-lg font-semibold transition-colors ${itemNoCarrinho(p.id, 'produto') ? 'bg-fbz-primary text-white' : 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-2 border-fbz-primary/20'}`}>
              ...
              {tabAtiva === 'planos' && planos.map((p) => (
                <div key={p.id} onClick={() => toggleCarrinho({ id: p.id, tipo: 'plano', nome: p.nome, preco: p.valor_total })} className={`card cursor-pointer border-2 ${itemNoCarrinho(p.id, 'plano') ? 'border-fbz-primary bg-fbz-primary/5' : 'border-blue-300'}`}>
                  <h3 className="font-bold text-lg">{p.nome}</h3>
                  <p className="text-sm text-muted-foreground mt-1">{p.itens_inclusos}</p>
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-lg font-bold text-fbz-gold">R$ {p.valor_total.toFixed(2)}</span>
                    <button className={`px-6 py-2 rounded-lg font-semibold transition-colors ${itemNoCarrinho(p.id, 'plano') ? 'bg-fbz-primary text-white' : 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-2 border-fbz-primary/20'}`}>
            ...
            {/* CARRINHO */}
            {carrinho.length > 0 && (
              <div className="card bg-fbz-primary/5 border-fbz-primary">
                ...
                {carrinho.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center mb-2">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-bold text-fbz-gold">R$ {item.preco.toFixed(2)}</span>
                ...
                <div className="pt-4 border-t border-border flex justify-between items-center">
                  ...
                  <span className="font-bold text-2xl text-fbz-gold">R$ {calcularTotal().toFixed(2)}</span>
                </div>
        ...
        {etapa === 'agendamento' && (
          <>
            <button onClick={() => setEtapa('selecao')} className="text-fbz-primary hover:underline mb-4">
              ← Voltar para seleção
            </button>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm">
                <div className="flex items-center space-x-4">
                  ...
                  <button
                    ...
                    className="ml-auto text-sm text-fbz-primary hover:underline"
                  >
                    Trocar
                  </button>
                </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full p-4 rounded-xl border-2 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 hover:border-fbz-primary dark:hover:border-fbz-accent transition-all text-left"
                  >
                  ...
                  {barbeiros.map((b) => (
                    <button
                      ...
                      className="w-full p-4 rounded-xl border-2 border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 hover:border-fbz-primary dark:hover:border-fbz-accent transition-all flex items-center space-x-4"
                    >
                      ...
                      <span className="text-fbz-primary dark:text-fbz-accent font-medium">Ver mais</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                {loadingHorarios ? (
                  <div className="flex items-center justify-center py-8">
                    <Loader2 className="w-6 h-6 animate-spin text-fbz-primary" />
                  </div>
                ) : horarios.length === 0 ? (
                              ...
                              <button
                                ...
                                className={`p-4 rounded-xl border-2 transition-all font-bold text-lg ${
                                  horarioSelecionado === h
                                    ? 'border-fbz-primary bg-fbz-primary text-white'
                                    : 'border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white hover:border-fbz-primary dark:hover:border-fbz-accent'
                                }`}
                              >
                                {h}
                              </button>
